 ...
 CanvasRenderingContext2D.prototype.wrapText = function (text, x, y, maxWidth, lineHeight) {
     ...
     for (var n = 0; n < arrText.length; n++) {
         ...
         if (testWidth > maxWidth && n > 0)
				 ...
				 else if(arrText[n] == "/" && arrText[n+1] == "n")
				 {
					 context.fillText(line, x, y);
					 n += 1;
					 line = '';
...
function text_atPos(xFn, yFn, text, font, fillStyle, alpha, lineHeight=75) {
  alpha || (alpha = 1.0);
  return function (target) {
    ...
    // context.fillText(text, xFn(target, metrics, context), yFn(target, metrics, context));
		context.wrapText(text, xFn(target, metrics, context), yFn(target, metrics, context), (context.canvas.width-200), lineHeight);
    context.restore();
    return target;
  };
}
...
function text_lowerRight(text, font, fillStyle, alpha, y) {
  return text_atPos(function (target, metrics) {
    return target.width - (metrics.width + 70);
  }, function (target) {
    return y || target.height - 70;
...
function text_center(text, font, fillStyle, alpha, lineHeight=75) {
  ...
  }, text, font, fillStyle, alpha, lineHeight);
}
 ...
 * @property {Number} encoderOptions - specify the image compression quality from 0 to 1 (default 1)
